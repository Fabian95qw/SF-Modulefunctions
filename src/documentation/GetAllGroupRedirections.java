...
import de.vertico.starface.persistence.connector.PersonAndAccountHandler.AccountType;
...
public class GetAllGroupRedirections implements IBaseExecutable
{
	// ##########################################################################################
	...
	public void execute(IRuntimeEnvironment context) throws Exception
	{
		...
		PersonAndAccountHandler PAH = (PersonAndAccountHandler) context.provider().fetch(PersonAndAccountHandler.class);
		GroupBusinessObject GBO = context.provider().fetch(GroupBusinessObject.class);

		for (Integer I : PAH.getAccountIdsByAccountType(AccountType.GROUP))
		{
			ExtendedGroup EUG = GBO.getGroupByAccountId(I);
			Map<String, Object> Map = new HashMap<String, Object>();
			
			Map.put("name", Objects.nullSafeToString(EUG.getDescription()));
			Map.put("loginid", Objects.nullSafeToString(EUG.getLoginNumber()));

			List<Map<String, String>> RedirectionList = new ArrayList<Map<String, String>>();

			List<RedirectSetting> Always = RBO.getRedirectSettings(EUG.getAccountId(), RedirectSettingType.ALWAYS, "",
					RedirectSettingProperties.calledNumber, OrderDirection.ASCENDING);
			List<RedirectSetting> Busy = RBO.getRedirectSettings(EUG.getAccountId(), RedirectSettingType.BUSY, "",
					RedirectSettingProperties.calledNumber, OrderDirection.ASCENDING);
			List<RedirectSetting> Timeout = RBO.getRedirectSettings(EUG.getAccountId(), RedirectSettingType.TIMEOUT, "",
					RedirectSettingProperties.calledNumber, OrderDirection.ASCENDING);
