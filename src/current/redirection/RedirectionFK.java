...
@Function(name="RedirectionFK", visibility=Visibility.Private, description="")
public class RedirectionFK implements IBaseExecutable 
{
	//##########################################################################################		
	
	@InputVar(label="TargetUser/TargetGroup", description="Zielbenutzer/Gruppe, für die die Umleitungen umgesetzt werden.", type=VariableType.STARFACE_ACCOUNT)
	...
	@InputVar(label="RedirectSettingType", description="Welcher Umleitungstyp soll geändert werden. (ALWAYS == IMMER || BUSY == BESETZT || TIMEOUT == Zeitüberschr.", valueByReferenceAllowed=true)
	...
	@InputVar(label="TimeoutInternal", description="Zeit, bis zur Zeitüberschreitung (interne Rufnummer)(Wird nur bei Zeitüb. Umleitung benötigt)", type=VariableType.NUMBER)
	...
	{	
		...
		try
		{
			...
			case NONE:
				break;
			case VOICEMAILBOX: //Voicemailbox wird später speziell behandelt
				break;	
			case USER: //Wenn Ziel ein Benutzer ist, die Benutzerdaten auflösen
				ResolveUserData RU = new ResolveUserData();
				RU.accountId = ((Double) ITarget).intValue();
				RU.execute(context);
				TargetInternal = RU.primaryCallNumber; //Als ziel die Primäre Rufnummer des Benutzers nehmen
				break;
			case GROUP: //Wenn Ziel eine Gruppe ist, die Gruppendaten auflösen
				ResolveGroupData RG = new ResolveGroupData();
				RG.accountId = ((Double) ITarget).intValue();
				RG.execute(context);
				TargetInternal = RG.primaryCallNumber;  //Als ziel die Primäre Rufnummer der Gruppe nehmen
				break;

			case NUMBER: //Wenn Ziel eine Nummer ist, die Nummer nehmen.
		...
		try
		{
			...
			case USER:  //Wenn Ziel ein Benutzer ist, die Benutzerdaten auflösen
				...
				RU.execute(context);
				TargetExternal = RU.primaryCallNumber; //Als ziel die Primäre Rufnummer des Benutzers nehmen
				break;
			case GROUP: //Wenn Ziel eine Gruppe ist, die Gruppendaten auflösen
				ResolveGroupData RG = new ResolveGroupData();
				RG.accountId = ((Double) ETarget).intValue();
				RG.execute(context);
				TargetExternal = RG.primaryCallNumber;  //Als ziel die Primäre Rufnummer der Gruppe nehmen
				break;
		...
		//Aktuelle Umleitungseinstellungen Abrufen.
