...
@Function(visibility=Visibility.Private, name="SetFieldbyGUIName4Instance" , description="Set Value of GUI_ELEMENT")
public class SetFieldbyGUIName4Instance implements IBaseExecutable 
{
	...
	public void execute(IRuntimeEnvironment context) throws Exception {
		...
		if(MIPObject == null)
		{
			context.getLog().debug("ModuleInstanceProject is null!");
			return;
		}
		...
		for (Variable Var : VARList)
		{
			...
			{
				VariableChanged = SetValue(Value, Var,MIP, context);
				Success=true;
				break;
			}
		}	
	...
	public boolean SetValue(Object O, Variable Var, ModuleInstanceProject MIP, IRuntimeEnvironment context) throws IOException, ModuleException
	{
		String Value=null;
		switch(Var.getType())
		{
		case BOOLEAN:
			Value = CAST_BOOLEAN(O, context);
		...
		case STARFACE_GROUP:
		case STARFACE_USER:
			Value= CAST_STARFACE_ENTITY(O, context);
			//log.debug(Value +" <==> " + Var.getValue());
			if(!Var.getValue().equals(Value))
			{
				Var.setValue(Value);
		...
		case MAP:
			{
				...
				//context.getLog().debug(Values.toString() +" <==> " + MR.getMap().toString());
		...
		case LIST:
		{
			...
			if(Var.getValue() != null)
			{
				//context.getLog().debug(Var.getValue());
		...
		case TIPTEL_SOUND_FILE:
		{
			...
			String S = CAST_STRING(O, context);
			
			File F = new File(S);
			
			FileResource FR = new FileResource(F.getName(), FileTypeforValueType(Var.getType()) , FilenameUtils.getExtension(F.getName()));
			FR.setId(Var.getId());
	...
	private FileType FileTypeforValueType(VariableType Type)
	{
		switch(Type)
		{
			case FILE_RESOURCE:
			...
			case ASTERISK_SOUND_FILE:
				return FileType.ASTERISK_SOUND;
			case SNOM_SOUND_FILE:
				return FileType.SNOM_SOUND;
			case TIPTEL_SOUND_FILE:
				return FileType.TIPTEL_SOUND;
			default:
				return null;
		}
	}

	private String CAST_STARFACE_ENTITY(Object O, IRuntimeEnvironment context)
	{
		...
		context.getLog().debug("Unable to Cast: "+ O.getClass().toString());
		return "-1";
	}
	
	private String CAST_NUMBER(Object O, IRuntimeEnvironment context)
	{
		if (O instanceof String)
		{
			String S = (String)O;
	...
	private String CAST_STRING(Object O, IRuntimeEnvironment context)
	{
		if (O instanceof String)
		{
			String S = (String)O;
	...
	private String CAST_BOOLEAN(Object O, IRuntimeEnvironment context)
	{
